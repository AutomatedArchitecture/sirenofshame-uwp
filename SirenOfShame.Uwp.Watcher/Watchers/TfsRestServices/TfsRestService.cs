...
namespace TfsRestServices
{
    public class TfsRestService
    {
        ...
        {
            // todo: ability to specify a collection (replace null below with a parameter from web UI)
            var buildDefinitionsGrouped = await GetBuildDefinitionsGrouped(getProjectsArgs.Url, null, getProjectsArgs.UserName, getProjectsArgs.Password);
            // todo: keep them grouped and display as such in the ui
            var buildDefinitions = buildDefinitionsGrouped
                .SelectMany(i => i.Projects)
                .SelectMany(i => i.BuildDefinitions);
