    ...
    public class HudsonBuildStatus : BuildStatus
    {
        ...
        private void DetermineBuildStatus(XElement docRoot, bool treatUnstableAsSuccess)
        {
            ...
            else
            {
                ...
                BuildStatusEnum = string.Equals(building, "true", StringComparison.CurrentCultureIgnoreCase)
        ...
        private void DetermineChangeDetails(XElement docRoot, XElement changeSetItem)
        {
            ...
            {
                var causeElement = GetCause(docRoot);
        ...
        private XElement GetCause(XContainer element)
        {
            var action = element.Elements("action").FirstOrDefault();
            if (action == null)
            {
                BuildStatusMessage = string.Format(CultureInfo.CurrentCulture, "Could not find node 'action' in the project xml");
                return null;
            }
            var cause = action.Elements("cause").FirstOrDefault();
            if (cause == null)
            {
                BuildStatusMessage = string.Format(CultureInfo.CurrentCulture, "Could not find node 'cause' in the project xml");
                return null;
            }
            return cause;
        }
