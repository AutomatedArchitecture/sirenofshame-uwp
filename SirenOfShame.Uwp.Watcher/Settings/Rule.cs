...
namespace SirenOfShame.Uwp.Watcher.Settings
{
    ...
    public class Rule
    {
        ...
        {
            bool isBuildDefinitionMatch = BuildDefinitionId == null ||
                                          BuildDefinitionId == buildStatus.BuildDefinitionId;
        ...
        {
            if (TriggerType == TriggerType.BuildFailed && buildStatus.BuildStatusEnum == BuildStatusEnum.Broken)
                return true;
            ...
            if (TriggerType == TriggerType.BuildTriggered && buildStatus.BuildStatusEnum == BuildStatusEnum.InProgress)
                return true;
            if (TriggerType == TriggerType.SubsequentFailedBuild &&
                buildStatus.BuildStatusEnum == BuildStatusEnum.Broken && !newlyBroken) return true;
            if (TriggerType == TriggerType.SuccessfulBuild && buildStatus.BuildStatusEnum == BuildStatusEnum.Working &&
                !newlyFixed) return true;
            return false;
        }
