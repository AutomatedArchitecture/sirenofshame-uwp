...
namespace SirenOfShame.Uwp.Watcher.Watcher
{
    ...
    public class BuildStatus : BuildStatusBase
    {
        public BuildStatus()
        {
        ...
        public BuildStatusDto AsBuildStatusDto(DateTime now, IDictionary<string, BuildStatus> previousWorkingOrBrokenBuildStatus, SirenOfShameSettings settings)
        {
            ...
            SetDisplayName(settings, result);
            return result;
        }

        private void SetDisplayName(SirenOfShameSettings settings, BuildStatusDto buildStatusDto)
        {
            var person = settings.FindAddPerson(buildStatusDto.RequestedByRawName);
            buildStatusDto.RequestedByDisplayName = person == null ? buildStatusDto.RequestedByRawName : person.DisplayName;
        }
